...
typedef struct
{
    u32 framefilter;               /*Address based frame filter configuration*/
    u32 speed_duplex_select;       /* Link speed and duplex mode allowed to setup a link. */
    u8  mac_addr[6];                /* Station's MAC address */
    u8  phy_addr;                   /* Address of Ethernet PHY on MII management interface. */
    u8  tx_edc_enable;              /* Enable / disable error detection and correction for tx FIFOs */
    u8  rx_edc_enable;              /* Enable / disable error detection and correction for rx FIFOs */
    u8  preamble_length;            /* 4-bit Length of preamble field: default value is 0x7 */
    u8  hugeframe_enable;           /* Enable / disable huge frame support: default is disable 0 */
    u8  length_field_check;         /* Enable / disable length field checking */
    u8  pad_n_CRC;                  /* Enable / disable padding and appending CRC */
    u8  append_CRC;                 /* Enable / disable appending CRC */
    u8  fullduplex;                 /* Enable / disable full duplex: default is disable 0 */
    u8  loopback;                   /* Enable / disable loopback mode: default is disable 0 */
    u8  rx_flow_ctrl;               /* Enable / disable receiver flow control: default is disable 0 */
    u8  tx_flow_ctrl;               /* Enable / disable transmitter flow control: default is disable 0 */
    u8  min_IFG;                    /* 8-bit minimum inter-frame gap value */
    u8  btb_IFG;                    /* 7-bit back to back inter-frame gap value */
    u8  max_retx_tries;             /* 5-bit maximum retransmission tries value: default is 0xF */
    u8  excessive_defer;            /* Enable / disable transmission of packet that exceeded max collisions: default is disable 0 */
    u8  nobackoff;                  /* Enable / disable back off. default is disable 0 */
    u8  backpres_nobackoff;         /* Enable / disable back off in back pressure mode: default is disable 0 */
    u8  ABEB_enable;                /* Enable / disable arbitrary binary exponential back-off: default is disable 0 */
    u8  ABEB_truncvalue;            /* 4-bit alternative binary exponential back-off value: default is 0xA */
    u8  phyclk;                     /* 3-bit MGMT clock divider value */
    u8  supress_preamble;           /* Enable / disable preamble suppression at PHY: default is disable 0 */
    u8  autoscan_phys;              /* Enable / disable auto scanning of PHYs with programmed addresses: default is disable 0 */
    u16 max_frame_length;          /* Maximum frame length: default value is 0x0600(1536d) */
    u16 non_btb_IFG;               /* 14-bit non back to back inter-frame gap value */
    u16 slottime;                  /* 10-bit collision window value : default is 0x37 */
    u32 framedrop_mask;            /* 18-bit mask to drop frames based on receive statistics */
    u8   wol_enable;                /* Enable / disable WoL event detect feature */
    u8   aneg_enable;               /* Enable / disable Auto-negotiation in MSGMII module as well as phy (if PHY is in the system) */
} tse_cfg_t;
#if 0
/*******************************************************************************
