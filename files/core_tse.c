 ...
 *
 * CoreTSE 10/100/1000 Mbps Ethernet MAC bare metal software driver implementation.
 *
 * SVN $Revision: 9728 $
...
#if (TSE_PHY_INTERFACE != TSE_1000BASEX)
        TSE_phy_set_link_speed(this_tse,
                               this_tse->phy_addr,
                               cfg->speed_duplex_select);
...
)
{
...
#else
    link_up = TSE_phy_get_link_status(this_tse,
                                      this_tse->phy_addr,
                                      &link_speed,
                                      &link_fullduplex);
#endif
    ...
    if(link_up != TSE_LINK_DOWN)
    {
        ...
        if((phy_reg & BMCR_ANENABLE) && ((TSE_PHY_INTERFACE == TSE_SGMII) || (TSE_PHY_INTERFACE == TSE_1000BASEX)))
        {
...
)
{
    ...
    if((TSE_PHYADDR_MAXVAL >= phyaddr) && (TSE_PHYREGADDR_MAXVAL >= regaddr))
    {
        ...
        if(timeout != 0u)
        {
            read_val = HAL_get_32bit_reg_field(this_tse->base_addr,
                                               MIIMSTATUS_PHY_STATUS);
        }
    }
...
)
{
    ...
    if(this_tse->nb_available_rx_desc > 0)
    {
        ...
        this_tse->rx_desc_tab[next_rx_desc_index].pkt_start_addr =
                                                        (uint32_t)rx_pkt_buffer;

        this_tse->rx_desc_tab[next_rx_desc_index].caller_info = p_user_data;

        /* Set own flag of this descriptor to indicate ready for RX */
        this_tse->rx_desc_tab[next_rx_desc_index].pkt_size |=
                                                       DMA_DESC_EMPTY_FLAG_MASK;
...
)
{
    ...
    for(count = 0; count < (nb_addresses * MAC_ADDRESS_LENGTH); count+=MAC_ADDRESS_LENGTH)
    {
        ...
        else if((mac_addresses[count] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRH)>>16)) &&
                (mac_addresses[count+1] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRH)>>24)) &&
                (mac_addresses[count+2] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRL))) &&
                (mac_addresses[count+3] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRL)>>8)) &&
                (mac_addresses[count+4] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRL)>>16)) &&
                (mac_addresses[count+5] == (uint8_t)(HAL_get_32bit_reg(this_tse->base_addr, STADDRL)>>24)))
        {
    ...
    reg_value = (uint32_t)((hash_unicast_set << FPC_HASH_UNICAST_SHIFT) |
                        (hash_mulicast_set << FPC_HASH_MULTICAST_SHIFT) |
                        (unicast_set << FPC_UNICAST_SHIFT) |
                        (broadcast_set << FPC_BROADCAST_SHIFT));
 ...
 * This informs the received packet size to the application and relinquishes the
 * packet buffer from the associated DMA descriptor.
 */
static void
rxpkt_handler
(
...
)
{
    ...
    do
    {
        ...
        else
        {
            ...
            empty_flag = this_tse->tx_desc_tab[index].pkt_size &
                         DMA_DESC_EMPTY_FLAG_MASK;
...
static void tx_desc_ring_init(tse_instance_t * this_tse)
{
    ...
    for(inc = 0; inc < TSE_TX_RING_SIZE; ++inc)
    {
        ...
        {
            this_tse->tx_desc_tab[inc].next_desriptor =
                                            (uint32_t)&this_tse->tx_desc_tab[0];
        }
        else
        {
            this_tse->tx_desc_tab[inc].next_desriptor =
                                      (uint32_t)&this_tse->tx_desc_tab[inc + 1];
        }
...
static void rx_desc_ring_init(tse_instance_t * this_tse)
{
    ...
    for(inc = 0; inc < TSE_RX_RING_SIZE; ++inc)
    {
        ...
        {
            this_tse->rx_desc_tab[inc].next_desriptor =
                                            (uint32_t)&this_tse->rx_desc_tab[0];
        }
        else
        {
            this_tse->rx_desc_tab[inc].next_desriptor =
                                      (uint32_t)&this_tse->rx_desc_tab[inc + 1];
        }
...
* Probe used PHY.
*
* return    PHY address. If PHY is not found, function returns 255.
*/
static uint8_t
phy_probe
(
