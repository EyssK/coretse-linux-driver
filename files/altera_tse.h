...
/* Microchip Triple-Speed Ethernet MAC driver
...
#define MICROCHIP_TSE_RESOURCE_NAME	"core_tse"
...
struct coretse_mac {
	...
	u32 half_duplex;
	u32 frm_length;
	u32 control_frame_extended_parameter;
	u32 control_frame_parameter;
	u32 test;
	...
	/* Number of control frame received */
	u32 if_in_control_pkts;
	/* Number of pause frame received */
	u32 if_in_pause_pkts;
	/* Number of unknown opcode packet received */
	u32 if_in_unkown_pkts;
	/* Frame received with an alignment error */
	u32 alignment_errors;
	/* Frame received where size does not match 802.3 size */
	u32 ether_stats_802_3_errors;
	/* Increment each time a valid carrier was present and an invalid data 
	 * symbol was detected */
	u32 ether_stats_code_errors;
	/* Increment each time a false carrier is detected during idle */
	u32 ether_stats_false_carrier_errors;
	...
	/* Number of pause frame transmitted */
	u32 if_out_pause_pkts;
	/* Number of transmitted deferred packet */
	u32 ether_stats_deffered;
	u32 ether_stats_excessive_defferal;
	/* Number of transmitted collision packet */
	u32 ether_stats_single_collision;
	u32 ether_stats_multiple_collision;
	u32 ether_stats_late_collision;
	u32 ether_stats_excessive_collision;
	u32 ether_stats_total_collision;
	
	u32 reserved1;
	
	/* Counts the number of transmitted dropped packets */
	u32 ether_stats_tx_drop_events;
	/* Too long frames with CRC error */
	u32 ether_stats_tx_jabbers;
	/* Number of frames transmitted with a CRC error */
	u32 frames_check_sequence_tx_errors;
	/* Number of control frame transmitted */
	u32 if_out_control_pkts;
	/* The number of frames transmitted that are longer than the
	 * value configured in the frm_length register */
	u32 ether_stats_oversize_tx_pkts;
	/* Number of packets received with less than 64 bytes. */
	u32 ether_stats_undersize_tx_pkts;
	/* Too short frames with CRC error */
	u32 ether_stats_tx_fragments;
...
#define tse_csroffs2(a) (offsetof(struct coretse_mac, a))
...
#define CORETSE_DMA 1
#define CORETSE_NODMA 2

/* standard DMA interface for SGDMA and MSGDMA */
struct coretse_dmaops {
	int coretse_dtype;
	int dmamask;
